...
class Auth extends Component {
   ...
   */
  // 사용자로부터 개인키를 입력받고 해당 개인키를 바탕으로 caver package를 사용하여 wallet instance 생성
  integrateWallet = (privateKey) => {
    const walletInstance = caver.klay.accounts.privateKeyToAccount(privateKey)
    caver.klay.accounts.wallet.add(walletInstance)
    // session에 wallet 정보를 설정하여 로그인에 편리하게끔 생성
  ...
  renderAuth = () => {
    ...
    // 즉 로그인이 된 상태에서는 component(사용자 UI)가 if 아래와 같이 변경됨
    if (walletInstance) {
      return (
        <Fragment>
    ...
    return (
      <Fragment>
        {accessType === 'keystore'
          ...
          // 즉 accessType != 'keystore'일 때 => private key로 로그인할 때
          : (
            <Fragment>
